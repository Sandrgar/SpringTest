...
public class AutoreController {
	...
	@GetMapping("/{id}")
	public AutoreEntity getAuthor(@PathVariable Long id) {
		return autoreService.getAutore(id);
	}
	
	@PostMapping
	public AutoreEntity insertAuthor(@RequestBody AutoreDto autoreDto) {
		return autoreService.insertAutore(autoreDto);
	}
	
	@PutMapping("/{id}")
	public AutoreEntity updateAuthor(@PathVariable Long id, @RequestBody AutoreDto authorDto) {
